...
class TaskonomyNetworkStudent(nn.Module):
    
    def __init__(self,
                 out_channels=3,
                 task1 = 'depth_euclidean',
                 task2 = 'edge_occlusion',
                 eval_only=True,
                 load_encoder_path=None,
                 load_decoder1_path=None,
                 load_decoder2_path=None,
                 model_dir=None,
        ...
        '''
        super(TaskonomyNetworkStudent, self).__init__()
        self.encoder = TaskonomyEncoder(eval_only=eval_only, train=True)
        self.encoder.normalize_outputs = False
        self.weights = torch.nn.Parameter(torch.tensor([0.5, 0.5], requires_grad=True))
        self.decoder1 = None
        self.decoder2 = None
        w_fea = torch.rand(2, 1024, 16, 16, requires_grad=True)
        self.weight_fea = torch.nn.Parameter(w_fea)
        w_rep = torch.rand(2, 8, 16, 16, requires_grad=True)
        self.weight_rep = torch.nn.Parameter(w_rep)
        self.is_decoder_mlp = is_decoder_mlp
        self.apply_tanh = apply_tanh


        if out_channels is not None:
            out_channels1 = TASKS_TO_CHANNELS[task1]
            apply_tanh1 = (task1 not in DONT_APPLY_TANH_TASKS)
            is_decoder_mlp1 = (task1 not in PIX_TO_PIX_TASKS)
            self.decoder1 = TaskonomyDecoder(
                                out_channels=out_channels1,
                                is_decoder_mlp=is_decoder_mlp1,
                                apply_tanh=apply_tanh1, 
                                eval_only=eval_only)
            out_channels2 = TASKS_TO_CHANNELS[task2]
            apply_tanh2 = (task2 not in DONT_APPLY_TANH_TASKS)
            is_decoder_mlp2 = (task2 not in PIX_TO_PIX_TASKS)
            self.decoder2 = TaskonomyDecoder(
                                out_channels=out_channels2,
                                is_decoder_mlp=is_decoder_mlp2,
                                apply_tanh=apply_tanh2, 
                                eval_only=eval_only)
        
        if load_encoder_path is not None:
            self.load_encoder(load_encoder_path, model_dir, progress)
        
        if load_decoder1_path is not None:
            self.load_decoder1(load_decoder1_path, model_dir, progress)

        if load_decoder2_path is not None:
            self.load_decoder2(load_decoder2_path, model_dir, progress)
    ...
    def load_decoder1(self, url, model_dir=None, progress=True):
        checkpoint = torch.utils.model_zoo.load_url(url, model_dir=model_dir, progress=progress)
        return self.decoder1.load_state_dict(checkpoint['state_dict'])
    
    def load_decoder2(self, url, model_dir=None, progress=True):
        checkpoint = torch.utils.model_zoo.load_url(url, model_dir=model_dir, progress=progress)
        return self.decoder2.load_state_dict(checkpoint['state_dict'])
    ...
    def forward_te(self, x):
        encoder_rep, encoder_mia = self.encoder.forward_feature_te(x)
        return self.decoder1(encoder_rep), self.decoder2(encoder_rep), encoder_rep, encoder_mia
    
    def get_last_shared_layer(self):
        return self.encoder.compress1
...
class TaskonomyNetwork(nn.Module):
    ...
    def forward_te(self, x):
        encoder_rep, encoder_mia = self.encoder.forward_feature_te(x)
        return self.decoder(encoder_rep), encoder_rep, encoder_mia
...
class TaskonomyEncoder(nn.Module):
    ...
    def forward_feature_te(self, x):
        ...
        x = self.layer1(x)
        x = self.layer2(x)
        mia_feature = self.layer3(x)
        x = self.layer4(mia_feature)
