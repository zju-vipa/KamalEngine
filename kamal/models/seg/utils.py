...
class conv2DGroupNorm(nn.Module):
    def __init__(
        ...
        self.cg_unit = nn.Sequential(
            conv_mod, nn.GroupNorm(n_groups, int(n_filters)))
...
class conv2DGroupNormRelu(nn.Module):
    def __init__(
        ...
        self.cgr_unit = nn.Sequential(
            conv_mod, nn.GroupNorm(n_groups, int(
    ...
    def __init__(self, in_size, out_size, is_batchnorm):
        ...
        if is_batchnorm:
            self.conv1 = nn.Sequential(
                nn.Conv2d(in_size, out_size, 3, 1, 0), nn.BatchNorm2d(
                    out_size), nn.ReLU()
            )
            self.conv2 = nn.Sequential(
                nn.Conv2d(out_size, out_size, 3, 1, 0), nn.BatchNorm2d(
                    out_size), nn.ReLU()
            )
        else:
            self.conv1 = nn.Sequential(
...
class segnetUp2(nn.Module):
    def __init__(self, in_size, out_size, positive=True):
...
class linknetUp(nn.Module):
    def __init__(self, in_channels, n_filters, stride=1):
        ...
        # B, 2C, H, W -> B, C/2, H, W
        self.convbnrelu1 = conv2DBatchNormRelu(
            in_channels, in_channels // 4, k_size=1, stride=1, padding=0
        )

        # B, C/2, H, W -> B, C/2, H, W
        self.deconvbnrelu2 = deconv2DBatchNormRelu(
            in_channels // 4, in_channels // 4, k_size=3, stride=stride, padding=1, output_padding=int(stride == 2)
        )

        # B, C/2, H, W -> B, C, H, W
        self.convbnrelu3 = conv2DBatchNormRelu(
            in_channels // 4, n_filters, k_size=1, stride=1, padding=0
        )
...
def interp(input, output_size, mode="bilinear"):
    ...
    h = torch.arange(0, oh, dtype=torch.float,
                     device=input.device) / (oh - 1) * 2 - 1
    w = torch.arange(0, ow, dtype=torch.float,
                     device=input.device) / (ow - 1) * 2 - 1
