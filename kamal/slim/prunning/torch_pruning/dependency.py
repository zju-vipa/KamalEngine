...
class _OffsetIndexTransform(object):
    def __init__(self, offset=None, reverse=False):
        self.offset = offset
        self.reverse = reverse

    def __call__(self, idxs):
        if self.reverse:
            new_idxs = [i-self.offset for i in idxs if i>=self.offset]
        else:
            new_idxs = [i+self.offset for i in idxs]
        return new_idxs
...
class Node(object):
    def __init__(self, module, grad_fn, node_name=None):
        ...
        self.outputs = []
        self.dependencies = []
        self.node_name = "%s (%s)"%(node_name, str(self.module)) if node_name is not None else str(self.module)
...
class Dependency(object):
    ...
    def __eq__(self, other):
        return (((self.trigger is None) or self.trigger == other.trigger) and
...
class DependencyGraph(object):
    ...
    def build_dependency( self, model, example_inputs ):
        ...
        self.module_to_node = self._obtain_forward_graph( model, example_inputs )
    ...
    def _obtain_forward_graph(self, model, example_inputs):
    ...
    def _set_fc_index_transform(self, fc_node: Node):
        ...
        global stride
        stride = 1
        def _travserse_graph(node):
            global stride
            if node in visited:
                return
            visited.add(node)
            if isinstance( node.module, (nn.Conv2d, nn.BatchNorm2d) ):
                oc = node.module.out_channels if node.type==OPTYPE.CONV else node.module.num_features
                if oc!=fc_in_features: # flatten or globalpooling ?
                    stride = fc_in_features // oc
                return
            elif node.type == OPTYPE.LINEAR :
                return
            else:
                for in_node in node.inputs:
                    _travserse_graph( in_node )
    ...
    def _set_concat_index_transform(self, cat_node: Node):
        ...
        for i, in_node in enumerate(cat_node.inputs):
            for dep in cat_node.dependencies:
                if dep.broken_node == in_node:
                    dep.index_transform = _OffsetIndexTransform( offset=offsets[i], reverse=True )

            for dep in in_node.dependencies:
                if dep.broken_node == cat_node:
                    dep.index_transform = _OffsetIndexTransform( offset=offsets[i], reverse=False )