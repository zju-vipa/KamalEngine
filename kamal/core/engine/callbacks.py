...
class ValidationCallback(CallbackBase):
    ...
    def after_step(self):
        ...
        trainer.history.put_scalars(**utils.flatten_dict(results))
        print(results)
        trainer.logger.info("[val %s] Iter %d/%d: %s" %
                            (self.model_name, trainer.iter, trainer.max_iter, results))
            ...
            for k, v in results.items():
                if isinstance(v, Iterable):  # skip non-scalar value
                    ...
                    trainer.viz.line([v, ], [trainer.iter, ], win="%s:%s" % (
                        self.model_name, k), update='append', opts={'title': "%s:%s" % (self.model_name, k)})
...
class VisualizeHistoryImagesCallbacks(CallbackBase):
    ...
    def after_step(self):
        ...
        for img_name in name_list:
            ...
            trainer.viz.images(images, nrow=nrow, win=img_name,
                               opts={'title': img_name})
