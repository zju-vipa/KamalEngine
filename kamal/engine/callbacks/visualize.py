...
class VisualizeOutputs(Callback):
    ...
    @staticmethod
    def get_prepare_fn(pred_fn=lambda x: x):
...
class VisualizeSegmentation(VisualizeOutputs):
    def __init__(
        ...
        if prepare_fn is None:
            prepare_fn = VisualizeOutputs.get_prepare_fn(pred_fn=lambda x: x.max(1)[1])
...
class VisualizeDepth(VisualizeOutputs):
    def __init__(
        ...
        if prepare_fn is None:
            prepare_fn = VisualizeOutputs.get_prepare_fn(pred_fn=lambda x: x)
        if decode_fn is None:
