...
def _assert_same_type(layers, layer_type=None):
    ...
    assert all(isinstance(l, layer_type) for l in layers)


def _get_layers(model_list):
    submodel = []
    for model in model_list:
        submodel.append(model.modules())
...
def combine_models(models, combine_parser):
    ...
    **Parameters:**
    ...
    def _recursively_combine(module):
        ...
        combined_module = combine_parser(module_output.link)
    ...
    combined_model = deepcopy(teacher_models[0])

    for combined_layer, layers in zip(combined_model.modules(), _get_layers(teacher_models)):
        combined_layer.link = layers  # link to teachers
