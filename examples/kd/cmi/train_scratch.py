...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.logger = kamal.utils.logger.get_logger(log_name, output='checkpoints/scratch/log-%s-%s.txt'%(args.dataset, args.model))
    if args.rank<=0:
    ...
    evaluator = kamal.slim.distillation.data_free.evaluators.classification_evaluator(val_loader)
...
def train(train_loader, model, criterion, optimizer, args):
    global best_acc1
    loss_metric = kamal.metrics.RunningLoss(nn.CrossEntropyLoss(reduction='sum'))
