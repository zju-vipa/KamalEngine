...
def train( cur_epoch, criterion, model, optim, train_loader, device, scheduler=None, print_interval=10):
    ...
    for cur_step, sample_batched in enumerate( train_loader ):
        ...
        if (cur_step+1)%print_interval==0:
            ...
            vis_preds = torch.max(outputs, 1)[1].cpu().data.numpy()
            vis_preds = np.expand_dims(vis_preds, axis=1) * 19
...
def validate( model, loader, device, metrics):
    ...
    with torch.no_grad():
        for i, (images, labels, depths, normals, masks) in  tqdm(enumerate(loader)):
            ...
            outputs = model(images)
            preds = outputs.data.max(dim=1)[1].cpu().numpy()
...
def get_parser():
    ...
    parser.add_argument("--ckpt", type=str, default='./checkpoints/seg/')
    return parser
...
def main():
    ...
    criterion = torch.nn.NLLLoss2d(ignore_index=255)
