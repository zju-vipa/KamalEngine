...
def main():
    ...
    train_dst = vision.datasets.NYUv2( 
        '../data/NYUv2', split='train', target_type='depth', transforms=sT.Compose([
            ...
            sT.Multi( sT.Normalize( mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225] ), sT.Lambda( lambd=lambda x: x/1e3 ) )
        ]) )
    ...
    metric = metrics.StandardTaskMetrics.monocular_depth()
    evaluator = engine.evaluator.BasicEvaluator( data_loader=val_loader, metric=metric, progress=False )
    task = engine.task.StandardTask.monocular_depth()
