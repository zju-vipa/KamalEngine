...
# Please refer to https://github.com/VainF/nyuv2-python-toolkit for data preparation
def main():
    # PyTorch Part
    ...
    # KAE Part
    metric = kamal.predefined.predefined_metrics.monocular_depth()
    evaluator = engine.evaluator.Evaluator( metric=metric, dataloader=val_loader,  )
    criterion = nn.MSELoss()
    trainer = engine.trainer.BasicTrainer( 
        logger=kamal.utils.logger.get_logger('nyuv2_depth'), 
        tb_writer=SummaryWriter( log_dir='checkpoints/tb/nyuv2_depth-%s'%( time.asctime().replace( ' ', '_' ) ) ) 
    )
    ...
    trainer.add_callback( 
        ...
        callbacks=callbacks.MetricsLogging(keys=(trainer.TOTAL_LOSS_METRIC, trainer.LR_METRIC, trainer.STEP_TIME_METRIC)))
    trainer.add_callback(
