...
def main():
    ...
    TOTAL_ITERS=len(train_loader) * 200 # 200 epochs
    ...
    # KAE Part
    # prepare evaluator
    evaluator = kamal.predefined.predefined_evaluator.classification_evaluator(val_loader)
    criterion = nn.CrossEntropyLoss()
    trainer = engine.BasicTrainer( 
        logger=kamal.utils.logger.get_logger('cifar100'), 
        tb_writer=SummaryWriter( log_dir='checkpoints/tb/cifar100-%s'%( time.asctime().replace( ' ', '_' ) ) ) 
    )
    ...
    # run
    trainer.run(start_iter=0, max_iter=TOTAL_ITERS, epoch_length=len(train_loader))
