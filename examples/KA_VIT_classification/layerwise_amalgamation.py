...
class AmalBlock(nn.Module):     #Figure 2
    def __init__(self, cs, cts):
        ...
        self.enc = nn.Conv2d(in_channels=sum(self.cts), out_channels=self.cs, kernel_size=1, stride=1, padding=0,
                             bias=True)      #第一个蓝箭头，sum(self.cts) = 2C1, self.cs = C2
        self.fam = nn.Conv2d(in_channels=self.cs, out_channels=self.cs, kernel_size=1, stride=1, padding=0, bias=True)  #FAM层
        self.dec = nn.Conv2d(in_channels=self.cs, out_channels=sum(self.cts), kernel_size=1, stride=1, padding=0,
                             bias=True)      #第二个蓝箭头

    def forward(self, fs, fts):
        rep = self.enc(torch.cat(fts, dim=1))    #cat:concatnate，拼接，dim=1是横着拼。将老师的feature拼接起来
        _fts = self.dec(rep)
        _fts = torch.split(_fts, self.cts, dim=1)
        _fs = self.fam(fs)   # teacher1和teacher2的token拼一起，与FC后的student token进行loss，fam相当于FC
        return rep, _fs, _fts
    # 先训练完两个老师，然后只训练学生，老师的weight要保存下来，每次训练学生时的某个图像都feed进老师里，然后做loss，学生训练也是先196，然后*2，做loss，输入到transformer
...
class LayerWiseAmalgamator(Engine):
    ...
    def setup(
        ...
        for group, C in zip(layer_groups, layer_channels):      #一个group就是一组老师网络的同一层的集合，C是channel，大小一样
            hooks = [FeatureHook(layer) for layer in group]     #hook可以从外部获取Module的中间输出结果
            amal_block = AmalBlock(cs=C[0], cts=C[1:]).to(self.device).train()   #set the module to training mode
    ...
    def run(self, max_iter, start_iter=0, epoch_length=None):
        ...
        for block, _, _ in self._amal_blocks:
            block_params.extend(list(block.parameters()))    # for block in teacher.blocks   para.extend(...)
        if isinstance(self.optimizer, torch.optim.SGD):
            self._amal_optimimizer = torch.optim.SGD(block_params, lr=self.optimizer.param_groups[0]['lr'],
                                                     momentum=0.9, weight_decay=1e-4)
        else:
            self._amal_optimimizer = torch.optim.Adam(block_params, lr=self.optimizer.param_groups[0]['lr'],
                                                      weight_decay=1e-4)
        self._amal_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self._amal_optimimizer, T_max=max_iter)  #学习率调整策略

        with set_mode(self.student, training=True), \
                set_mode(self.teachers, training=False):
            super(LayerWiseAmalgamator, self).run(self.step_fn, self._dataloader, start_iter=start_iter,
