...
from kamal import engine, utils, slim, vision

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--lr', type=float, default=0.05)
    parser.add_argument('--t_model_path', type=str,
                        default='~/checkpoints/pretrained/cifar100_wrn_40_2.pth')
    ...
    viz = SummaryWriter(log_dir='run/distill_%s-%s' %
                        (args.distill, time.asctime().replace(' ', '_')))
    if args.distill == 'kd':
        optimizer = torch.optim.SGD(student.parameters(
        ), lr=args.lr, momentum=0.9, weight_decay=5e-4)
        distiller = slim.KDDistiller(logger, viz)
        distiller.setup(student=student, teacher=teacher, data_loader=train_loader,
                        optimizer=optimizer, T=args.T, gamma=args.gamma, alpha=args.alpha, device=device)
    if args.distill == 'attention':
        optimizer = torch.optim.SGD(student.parameters(
        ), lr=args.lr, momentum=0.9, weight_decay=5e-4)
        distiller = slim.AttentionDistiller(logger, viz)
        distiller.setup(student=student, teacher=teacher, data_loader=train_loader, optimizer=optimizer, T=args.T, 
    ...
    if args.distill == 'nst':
        ...
        distiller = slim.NSTDistiller(logger, viz)
        distiller.setup(student=student, teacher=teacher, data_loader=train_loader, optimizer=optimizer, T=args.T, 
                        gamma=args.gamma, alpha=args.alpha, beta=args.beta, stu_hooks=stu_hooks, tea_hooks=tea_hooks, out_flags=out_flags, device=device)
    if args.distill == 'sp':
        optimizer = torch.optim.SGD(student.parameters(
        ), lr=args.lr, momentum=0.9, weight_decay=5e-4)
        distiller = slim.SPDistiller(logger, viz)
        distiller.setup(student=student, teacher=teacher, data_loader=train_loader, optimizer=optimizer, T=args.T, 
                        gamma=args.gamma, alpha=args.alpha, beta=args.beta, stu_hooks=stu_hooks, tea_hooks=tea_hooks, out_flags=out_flags, device=device)
    if args.distill == 'rkd':
        optimizer = torch.optim.SGD(student.parameters(
        ), lr=args.lr, momentum=0.9, weight_decay=5e-4)
        distiller = slim.RKDDistiller(logger, viz)
        distiller.setup(student=student, teacher=teacher, data_loader=train_loader, optimizer=optimizer, T=args.T, 
                        gamma=args.gamma, alpha=args.alpha, beta=args.beta, stu_hooks=stu_hooks, tea_hooks=tea_hooks, out_flags=out_flags, device=device)
    if args.distill == 'pkt':
        optimizer = torch.optim.SGD(student.parameters(
        ), lr=args.lr, momentum=0.9, weight_decay=5e-4)
        distiller = slim.PKTDistiller(logger, viz)
        distiller.setup(student=student, teacher=teacher, data_loader=train_loader, optimizer=optimizer, T=args.T, 
                        gamma=args.gamma, alpha=args.alpha, beta=args.beta, stu_hooks=stu_hooks, tea_hooks=tea_hooks, out_flags=out_flags, device=device)
    if args.distill == 'svd':
        optimizer = torch.optim.SGD(student.parameters(
        ), lr=args.lr, momentum=0.9, weight_decay=5e-4)
        distiller = slim.SVDDistiller(logger, viz)
        distiller.setup(student=student, teacher=teacher, data_loader=train_loader, optimizer=optimizer, T=args.T, 
                        gamma=args.gamma, alpha=args.alpha, beta=args.beta, stu_hooks=stu_hooks, tea_hooks=tea_hooks, out_flags=out_flags, device=device)
    if args.distill == 'vid':
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        ...
        distiller = slim.VIDDistiller(logger, viz)
        distiller.setup(student=student, teacher=teacher, data_loader=train_loader, optimizer=optimizer, regressor_l=VIDRegressor_l, T=args.T, 
                        gamma=args.gamma, alpha=args.alpha, beta=args.beta, stu_hooks=stu_hooks, tea_hooks=tea_hooks, out_flags=out_flags, device=device)
    if args.distill == 'hint':
        ...
        distiller = slim.HintDistiller(logger, viz)
        distiller.setup(student=student, teacher=teacher, data_loader=train_loader, optimizer=optimizer, regressor=fitnet, hint_layer=args.hint_layer,
    ...
    if args.distill == 'cc':
        ...
        distiller = slim.CCDistiller(logger, viz)
        distiller.setup(student=student, teacher=teacher, data_loader=train_loader, optimizer=optimizer, embed_s=embed_s, embed_t=embed_t,
                                               T=args.T, gamma=args.gamma, alpha=args.alpha, beta=args.beta, stu_hooks=stu_hooks, tea_hooks=tea_hooks, out_flags=out_flags, device=device)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, len(train_loader)*args.epochs)
    distiller.add_callbacks([
        ...
        engine.callbacks.ValidationCallback(
            ...
            ckpt_tag='cifar100_%s' % (args.distill),
            verbose=False
        )
    ])
    distiller.run(0, len(train_loader)*args.epochs)
