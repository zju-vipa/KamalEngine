...
def main():
    # PyTorch Part
    model = vision.models.segmentation.deeplabv3_resnet50(num_classes=1, pretrained_backbone=True)
    ...
    trainer = engine.trainer.BasicTrainer( 
        ...
        tb_writer=SummaryWriter( log_dir='run/nyuv2_depth_deeplab-%s'%( time.asctime().replace( ' ', '_' ) ) ) 
    )
