...
def main():
    ...
    task = [ kamal.tasks.StandardTask.distillation(attach_to=[0,0]), 
    ...
    trainer = engine.trainer.KDTrainer( 
        ...
        tb_writer=SummaryWriter( log_dir='run/nyuv2_simple_kd-%s'%( time.asctime().replace( ' ', '_' ) ) ) 
    )
    ...
    trainer.add_callback( 
        ...
        callbacks=[ 
            callbacks.EvalAndCkpt(model=model, evaluator=evaluator, metric_name='rmse', metric_mode='min', ckpt_prefix='nyuv2_simple_kd'),
