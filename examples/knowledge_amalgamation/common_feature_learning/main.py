...
def main():
    ...
    trainer = amalgamation.CommonFeatureAmalgamator( 
        ...
        viz=SummaryWriter( log_dir='../run/cfl-%s'%( time.asctime().replace( ' ', '_' ) ) ) 
    )
    ...
    endpoints = [ ('layer4', 512) ]
    layer_groups = []
    layer_channels = []
    for endp, nch in endpoints:
        layer = [ getattr( student, endp ), getattr( car_teacher, endp ), getattr( aircraft_teacher, endp ) ]
        layer_groups.append( layer )
        layer_channels.append( [ nch, nch, nch ] )
