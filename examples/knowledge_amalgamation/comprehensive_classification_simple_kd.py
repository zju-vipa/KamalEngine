...
def main():
    ...
    trainer = amalgamation.CommonFeatureAmalgamator(  # KD can be achieved by scaling other losses to zero
        logger=utils.logger.get_logger('kd'), 
        tb_writer=SummaryWriter( log_dir='run/kd-%s'%( time.asctime().replace( ' ', '_' ) ) ) 
    )
    ...
    trainer.add_callback( 
        ...
        callbacks=[
            callbacks.EvalAndCkpt(model=student, evaluator=car_evaluator, metric_name='car_acc', ckpt_prefix='kd_car'),
            callbacks.EvalAndCkpt(model=student, evaluator=aircraft_evaluator, metric_name='aircraft_acc', ckpt_prefix='kd_aircraft'),
        ] )
    trainer.add_callback(
