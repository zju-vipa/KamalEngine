...
#         self.conv = nn.Conv2d(s_C, t_C, kernel_size=(1, 1))
...
class MultiConvReg(nn.Module):
    def __init__(self, s_shape, t_shape, num=2, use_relu=True):
        super(MultiConvReg, self).__init__()
        self.use_relu = use_relu
        ...
        if s_H == 2 * t_H:
            ...
            s = math.floor(t_H/s_H)
            self.conv = nn.ConvTranspose2d(s_C, t_C, kernel_size=(t_H - s * (s_H-1), t_W - s * (s_W-1)), stride=s, padding=0, output_padding=0)
        else:
            raise NotImplemented('student size {}, teacher size {}'.format(s_H, t_H))
        self.bn = nn.BatchNorm2d(t_C)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(t_C, t_C, kernel_size=(1,1))
...
class MultiLinearEmbed(nn.Module):
    """Linear Embedding"""
    def __init__(self, dim_in=1024, dim_out=128):
        super(MultiLinearEmbed, self).__init__()
        self.linear = nn.Linear(dim_in, dim_out)
