...
class ResNet(nn.Module):
    ...
    def forward(self, x, is_feat=False, preact=False, begin=0, end=100):
        if begin<=0:
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.relu(x)  # 32x32
...
if __name__ == '__main__':
    ...
    print("{} paramerters in total".format(sum(x.numel() for x in net.parameters())))
