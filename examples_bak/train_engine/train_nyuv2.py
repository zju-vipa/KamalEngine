...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        vision.datasets.NYUv2(args.data_root, split='train', transforms=sT.Compose([
    ...
    val_loader = torch.utils.data.DataLoader(
        vision.datasets.NYUv2(args.data_root, split='test', transforms=sT.Compose([
            sT.Multi(sT.ToTensor(normalize=True),sT.ToTensor(normalize=False, dtype=torch.long) ),
            sT.Multi(sT.Normalize( mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), None )
        ] )), batch_size=16, num_workers=2)

    # Prepare model
    model = vision.models.segmentation.segnet_vgg11_bn(num_classes=13, pretrained_backbone=True)
