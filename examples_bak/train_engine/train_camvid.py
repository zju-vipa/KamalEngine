...
from kamal import engine, metrics, vision, utils
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        vision.datasets.CamVid(args.data_root, split='trainval', transforms=sT.Compose([
    ...
    val_loader = torch.utils.data.DataLoader(
        vision.datasets.CamVid(args.data_root, split='test', transforms=sT.Compose([
            sT.Multi(sT.ToTensor(normalize=True),sT.ToTensor(normalize=False, dtype=torch.long) ),
            sT.Multi(sT.Normalize( mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), None )
        ] )), batch_size=16, num_workers=2)

    # Prepare model
    model = vision.models.deeplabv3plus_mobilenetv2(num_classes=11, pretrained_backbone=True)
    ...
    trainer.add_callbacks( [
        ...
        engine.callbacks.VisualizeSegmentationCallBack(
            ...
            denormalizer=utils.Denormalizer(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
